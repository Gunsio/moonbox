...
- Skill Picker now follows the selected UI language and user-facing handoff
  model: agent-backed rows are labeled as `Skill` with runner / skill / setup /
  command separated, built-in fallback rows are labeled as draft templates, and
  missing stars metadata no longer appears as `n/a` or `not configured`.
