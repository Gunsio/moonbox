...
- TUI Handoff Review preparation now runs in the background with a cancellable
  loading panel, opens at the top of the generated handoff, supports `gg` / `G`
  jumps for long reviews, and clamps overscroll so the body cannot blank out.
...
- Launch target selection no longer drives the agent runner. The TUI now keeps
  handoff skill, target agent, and local runner preference separate: Launch
  selects only the target agent, Settings owns the Codex-default runner
  preference, and generation uses `agent:<runner>:<skill>` from that preference.
- Real-session Launch now promotes an installed `handoff` skill before target
  confirmation instead of forcing users through the Skill Picker when the
  current capsule still points at a built-in draft compiler.
...
- Skill Picker now handles Enter while opened above Launch: selecting a handoff
  skill applies it and starts background Review generation in one step.
...
- Agent-backed Handoff Review now opens at the top of the generated Markdown and
  clamps modal scroll offsets, so `j/k` cannot overscroll into a blank body and
  `gg` is no longer required before normal scrolling.
