...
fn fixture_safe_command(binary: &str, test_name: &str) -> Command {
    ...
    for key in [
        ...
        "MOONBOX_CLAUDE_BIN",
        "MOONBOX_HERMES_BIN",
        "MOONBOX_LARK_CLI_BIN",
        "MOONBOX_LARK_DISABLE_OPEN",
        "MOONBOX_REDACTION",
...
#[test]
fn lark_handoff_export_dry_run_reports_missing_cli_without_remote_write() {
    let home = fixture_home("lark-export-missing");
    let missing = home.join("bin").join("missing-lark-cli");
    let json = output_json(
        moonbox_command("lark-export-missing")
            .args([
                "export",
                "--session",
                "codex-cxcp-design",
            ...
            .env("MOONBOX_LARK_CLI_BIN", &missing)
            .output()
            .expect("lark export dry-run"),
    );

    assert_eq!(json["dry_run"], true);
    assert_eq!(json["destination"], "lark");
    assert_eq!(json["mode"], "handoff");
    assert_eq!(json["execute_ready"], false);
    assert_eq!(json["lark_cli"]["state"], "missing");
    assert!(
        json["lark_cli"]["setup_command"]
            .as_str()
            .expect("setup command")
            .contains("setup install lark-cli")
    );
    assert!(json["risks"].as_array().expect("risks").iter().any(|risk| {
        risk.as_str()
            .expect("risk")
            .contains("Dry-run does not call")
    }));
}
...
cat <<'JSON'
{
  ...
  "capsule": {
    ...
    "source_cli": "codex",
    "target_cli": "hermes",
    "source_session": "codex-cxcp-design",
    "rewind_point": "evt-091 / selected rewind",
    "compiler": "lark-handoff-test",
    "handoff_label": "moonbox/handoff-test",
    "goal": "Generated by fake compiler.",
    "state": "handoff_ready",
    "decisions": ["The configured handoff runner executed before Lark export."],
    ...
    "redaction": {
      ...
      "external_compiler_disclosure": "disabled",
      "warnings": []
    }
  }
}
...
"##,
    ...
    let fake_lark = write_executable_script(
        &home,
        "bin/lark-cli",
        r#"#!/bin/sh
if [ "$1" = "--version" ]; then
  echo "lark-cli version 1.0.55"
  exit 0
fi
if [ "$1" = "docs" ] && [ "$2" = "+create" ]; then
  for arg in "$@"; do
    if [ "$arg" = "--help" ]; then
      echo "Usage: lark-cli docs +create --content TEXT --doc-format markdown --as user"
      exit 0
    fi
  done
  printf '%s\n' "$@" > "$MOONBOX_FAKE_LARK_ARGS"
  while [ "$#" -gt 0 ]; do
    if [ "$1" = "--content" ]; then
      shift
...
"#,
    ...
    let json = output_json(
        moonbox_command(test_name)
            ...
            .output()
            .expect("lark export execute"),
    );

    assert_eq!(json["plan"]["dry_run"], false);
    assert_eq!(json["plan"]["compiler"], "lark-handoff-test");
    assert_eq!(json["url"], "https://example.test/docx/moonbox");
    assert_eq!(json["browser_opened"], false);

    let args = fs::read_to_string(args_path).expect("fake lark args");
    assert!(args.contains("docs\n+create\n"));
