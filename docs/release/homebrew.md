...
2. Tag the accepted commit, for example `v0.1.5-beta.5`.
3. Stage local release artifacts:

```bash
scripts/release/stage-artifacts.sh --version 0.1.5-beta.5 --tag v0.1.5-beta.5
```
...
class Moonbox < Formula
  ...
  bottle do
    root_url "https://github.com/Gunsio/moonbox/releases/download/v0.1.5-beta.5"
    rebuild 1
  ...
  on_macos do
    on_arm do
      url "https://github.com/Gunsio/moonbox/releases/download/v0.1.5-beta.5/moonbox-0.1.5-beta.5-aarch64-apple-darwin.tar.gz"
      sha256 "<release-binary-sha256>"
    end

    on_intel do
      url "https://github.com/Gunsio/moonbox/releases/download/v0.1.5-beta.5/moonbox-0.1.5-beta.5-source.tar.gz"
  ...
  def install
    binary_root = if (buildpath/"bin/moonbox").exist?
      buildpath
    elsif (buildpath/"moonbox-0.1.5-beta.5-aarch64-apple-darwin/bin/moonbox").exist?
      buildpath/"moonbox-0.1.5-beta.5-aarch64-apple-darwin"
    end
