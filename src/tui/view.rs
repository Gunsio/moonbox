...
mod tests {
    ...
    fn settings_overlay_aligns_long_theme_status_columns() {
        ...
        app.set_ui_preferences_for_test(crate::core::config::UiPreferencesConfig {
            language: crate::core::config::UiLanguage::ZhHans,
            theme: crate::core::config::UiThemeName::Gruvbox,
        });
        app.handle_key(key(','));
        app.handle_key(key('j'));
        app.handle_key(key(' '));
        ...
        assert!(theme_line.contains("已保存 Gruvbox"), "{theme_line}");
        assert!(theme_line.contains("未保存"), "{theme_line}");

        let saved_index = theme_line.find("已保存 Gruvbox").expect("saved column");
        ...
        assert!(
            state_column >= saved_column + display_width("已保存 Gruvbox") + 2,
