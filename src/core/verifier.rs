...
fn file_references_check(capsule: &WorkCapsule) -> VerificationCheck {
    ...
    if !missing.is_empty() {
        return check(
            "semantic_file_refs",
            if is_agent_handoff_artifact(capsule) {
                VerificationStatus::Warn
            } else {
                VerificationStatus::Fail
            },
...
fn is_agent_handoff_artifact(capsule: &WorkCapsule) -> bool {
...
mod tests {
    ...
    #[test]
    fn agent_handoff_artifact_missing_file_reference_warns() {
        let data = data::workbench_data(CliTool::Codex, CliTool::Hermes).expect("data");
        let session = data
            .sessions
            .iter()
            ...
            .expect("source session");
        let mut capsule = data.capsule.clone();
        capsule.compiler = "agent:codex:handoff".into();
        capsule.handoff_artifact = Some("# Handoff\nGenerated by community skill.".into());
        capsule.raw_source_map = None;
        capsule.raw_refs.clear();
        capsule.coverage = Default::default();
        capsule
            .evidence
            .push("file:fixtures/adapters/missing-agent-artifact-file.md".into());
        ...
        assert!(report.checks.iter().any(|check| {
            check.name == "semantic_file_refs"
                && check.status == VerificationStatus::Warn
                && check.detail.contains("missing-agent-artifact-file.md")
        }));
    }
